...
int main()
{
    ...
    while (!file.eof()) {
        file >> s;
        is_rule = true;
        //Удаляем все символы, неявляющиеся русскими буквами, по краям слова.
        //s.back() возвращает последний символ строки.
        while (!((s[0] >= 'А') && (s[0] <= 'я') || (s[0] == 'Ё') || (s[0] == 'ё') || (s[0] == '\0')) || !((s[0] == '\0') || (s.back() >= 'А') && (s.back() <= 'я') || (s.back() == 'Ё') || (s.back() == 'ё')))
        {
            if (!((s[0] >= 'А') && (s[0] <= 'я') || (s[0] == 'Ё') || (s[0] == 'ё')))
                s.erase(0, 1);
            else
